...
use TotalProcessing\Opp\Gateway\Response\ScheduleHandler;
...
class ScheduleValidator extends AbstractValidator
{
    ...
    protected function getStatements(array $validationSubject): array
    {
        ...
        return [
            [
                'statement' => array_key_exists(ScheduleHandler::CLIENT_ID, $response)
                    && ($response[ScheduleHandler::CLIENT_ID] > 0),
                'errorCode' => VirtualErrorMessageMapper::DEFAULT_ERROR_CODE,
            ],
            [
                'statement' => array_key_exists(ScheduleHandler::FREQUENCY, $response)
                    && !empty($response[ScheduleHandler::FREQUENCY]),
                'errorCode' => VirtualErrorMessageMapper::DEFAULT_ERROR_CODE,
            ]
        ];
    }
